...
from knnbox.datastore.utils import build_faiss_index, load_faiss_index, load_faiss_index_subset
...
class Datastore:
    ...
    def __getitem__(self, name):
        r""" access  inner data
        Usage:
            Create a keys.npy and a vals.npy by Memmap
    ...
    @classmethod #可以直接类名.方法名调用，无需实例化
    ...
    def load_faiss_index_subset(self, filename, subset_ids, move_to_gpu=True, verbose=True):
        ...
        if not hasattr(self, "faiss_index") or self.faiss_index is None:
            self.faiss_index = {}
        self.faiss_index[filename] = load_faiss_index_subset(
                        path = index_path,
                        n_probe = 32,
