...
from knnbox.combiner.utils import calculate_knn_prob, calculate_combined_prob, calculate_ensemble_combined_prob
...
class Combiner:
    ...
    def get_ensemble_combined_prob(self, knn_prob_list, neural_model_logit, lambda_list, log_probs = False):
        r""" 
        strategy of combine probability of vanilla knn-mt
        If parameter `lambda_` is given, it will suppress the self.lambda_ 
        """
        return calculate_ensemble_combined_prob(knn_prob_list, neural_model_logit, lambda_list, log_probs)
