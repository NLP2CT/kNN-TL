...
def calculate_ensemble_combined_prob(knn_prob_list, neural_model_logit, lambda_list, log_probs):
    r""" 
    How vanilla knn-mt calculate the combining probability.
    """
    assert len(knn_prob_list)==len(lambda_list)
    neural_model_prob = F.softmax(neural_model_logit, dim=-1)

    ensemble_combined_probs = neural_model_prob * (1-sum(lambda_list))
    for knn_prob,lambda_ in zip(knn_prob_list,lambda_list):
        ensemble_combined_probs += knn_prob * lambda_

    # some extra infomation
    extra = {}
    extra["neural_probs"] = neural_model_prob
    extra["unlog_combined_probs"] = ensemble_combined_probs

    if log_probs:
        ensemble_combined_probs =  torch.log(ensemble_combined_probs)
    return ensemble_combined_probs, extra