...
@register_model("parent_syn_mt")
class ParentSYNMT(TransformerModel):
    r"""
    ...
    def add_args(parser):
        ...
        parser.add_argument("--retrive-datastore-path", type=str, metavar="STR",
                            help="the directory of save or load datastore")
    ...
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        r"""
        we override this function, replace the TransformerDecoder with ParentSYNMTDecoder
        """
        return ParentSYNMTDecoder(
            args,
            tgt_dict,
            embed_tokens,
...
class ParentSYNMTDecoder(TransformerDecoder):
    r"""
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        if args.knn_mode == "build_datastore":
            ...
            self.retrive_datastore = Datastore.load(args.retrive_datastore_path, load_list=["vals"])
            self.retrive_datastore.load_faiss_index("keys")
            self.retriever = Retriever(datastore=self.retrive_datastore, k=2047)
            if os.path.exists(args.knn_datastore_path+'/syn_retrivel.npy'):
                global_vars()["syn_retrivel_ind_dict"] = np.load(args.knn_datastore_path+'/syn_retrivel.npy',allow_pickle=True).item()
            else:
    ...
    def forward(
        ...
        if self.args.knn_mode == "build_datastore":
            ...
            self.retriever.retrieve(keys, return_list=["indices"])
            inds = set(self.retriever.results["indices"].cpu().numpy().reshape(-1).tolist())
            for i in inds:
                if i not in self.ind_dict.keys():
                    self.ind_dict[i] = 1
                    global_vars()["count"] += 1
                else:
                    self.ind_dict[i] += 1
...
"""
@register_model_architecture("parent_syn_mt", "parent_syn_mt@transformer")
def base_architecture(args):
    archs.base_architecture(args)

@register_model_architecture("parent_syn_mt", "parent_syn_mt@transformer_iwslt_de_en")
def transformer_iwslt_de_en(args):
    archs.transformer_iwslt_de_en(args)

@register_model_architecture("parent_syn_mt", "parent_syn_mt@transformer_wmt_en_de")
def transformer_wmt_en_de(args):
    archs.base_architecture(args)

# parameters used in the "Attention Is All You Need" paper (Vaswani et al., 2017)
@register_model_architecture("parent_syn_mt", "parent_syn_mt@transformer_vaswani_wmt_en_de_big")
def transformer_vaswani_wmt_en_de_big(args):
    archs.transformer_vaswani_wmt_en_de_big(args)

@register_model_architecture("parent_syn_mt", "parent_syn_mt@transformer_vaswani_wmt_en_fr_big")
def transformer_vaswani_wmt_en_fr_big(args):
    archs.transformer_vaswani_wmt_en_fr_big(args)

@register_model_architecture("parent_syn_mt", "parent_syn_mt@transformer_wmt_en_de_big")
def transformer_wmt_en_de_big(args):
    archs.transformer_vaswani_wmt_en_de_big(args)

# default parameters used in tensor2tensor implementation
@register_model_architecture("parent_syn_mt", "parent_syn_mt@transformer_wmt_en_de_big_t2t")
def transformer_wmt_en_de_big_t2t(args):
    archs.transformer_wmt_en_de_big_t2t(args)

@register_model_architecture("parent_syn_mt", "parent_syn_mt@transformer_wmt19_de_en")
