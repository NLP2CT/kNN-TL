...
class KnnTLVisual(TransformerFromPretrainedMTModel):
    r"""
    ...
    def add_args(parser):
        ...
        parser.add_argument("--build-datastore-path", type=str, metavar="STR",
                            help="the directory of save or load datastore")
...
class KnnTLVisualDecoder(TransformerDecoderFromPretrainedMT):
    r"""
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        if args.knn_mode == "build_datastore":
            ...
            if "build_datastore" not in global_vars():
                global_vars()["build_datastore"] = Datastore(args.build_datastore_path)  
            self.build_datastore = global_vars()["build_datastore"]
    ...
    def forward(
        ...
        if self.args.knn_mode == "build_datastore":
            ...
            if self.args.knn_keytype == "last_ffn_input":
                ...
                keys = select_keys_with_pad_mask(extra["last_ffn_input"], self.build_datastore.get_pad_mask())
            else:
                self.retriever.retrieve(x, return_list=["vals", "distances"])
            self.build_datastore["keys"].add(keys.half())
