...
def main(cfg: FairseqConfig) -> None:
    ...
    teacher_dir = criterion.teacher_dir
    aux_task = copy.deepcopy(cfg.task)
    aux_task.data = criterion.teacher_data
    teacher = checkpoint_utils.load_model_ensemble([teacher_dir])[0][0].to(trainer.device)
    teacher.train()
    ...
    while epoch_itr.next_epoch_idx <= max_epoch:
        ...
        # train for one epoch
        valid_losses, should_stop = train(cfg, trainer, task, epoch_itr, teacher=teacher)
        if should_stop:
            break
...
def train(
    cfg: DictConfig, trainer: Trainer, task: tasks.FairseqTask, epoch_itr, teacher
    ...
    for i, samples in enumerate(progress):
        with metrics.aggregate("train_inner"), torch.autograd.profiler.record_function(
            ...
            log_output = trainer.train_step(samples, teacher=teacher)
