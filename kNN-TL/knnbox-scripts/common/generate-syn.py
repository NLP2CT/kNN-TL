...
This file is copied from fairseq_cli/generate.py
    box make slight change about parsing args so that is
...
# parser.add_argument('--srclang', type=str, default='ca')
# parser.add_argument('--k', type=int, default=512)
# aargs = parser.parse_args()
# print(aargs)
# global_vars()["srclang"] = aargs.src
# global_vars()["k"] = str(aargs.k)

def main(cfg: DictConfig, override_args):
    ...
    if cfg.common_eval.results_path is not None:
        ...
        with open(output_path, "w", buffering=1, encoding="utf-8") as h:
            return _main(cfg, override_args, h)
    else:
        return _main(cfg, override_args, sys.stdout)
...
def _main(cfg: DictConfig, override_args, output_file):
    logging.basicConfig(
    ...
    for sample in progress:
        ...
        # print("len(global_vars()[syn_retrivel_ind_dict].keys())",len(global_vars()["syn_retrivel_ind_dict"].keys()))
    ...
    print("---len(global_vars()[syn_retrivel_ind_dict].keys())",len(global_vars()["syn_retrivel_ind_dict"].keys()))
    for kl in global_vars()["k_list"]:
        syn_retrieve_ids = list(global_vars()["syn_retrivel_ind_dict"][kl].keys())
        syn_retrieve_ids.sort()
        syn_retrieve_ids = np.array(syn_retrieve_ids)
        np.save(cfg.task.data+'/syn_retrieve_'+str(kl)+'k',syn_retrieve_ids)
    return scorer
